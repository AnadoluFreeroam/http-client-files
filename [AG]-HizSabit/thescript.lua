...
function cc()
	if (not isElement(myveh)) then
		...
		ccEnabled=false
		outputChatBox("#FFFFFFSabitleyici #2254ffEtkin#FFFFFF Degil.", 255,255,255,true)
		return false
	end
	...
	if (x<15) then
		...
		if (targetSpeedTmp > 3) then
			setElementSpeed(myveh, "c", targetSpeedTmp)
		end
	end
end
				...
				if targetSpeed > 4 then
					if (limit) then
						if in_array(getVehicleType(veh), allowedTypes) then
							...
							outputChatBox("#FFFFFFSabitleyici #2254ffEtkin. #FFFFFFHiziniz #2254ff"..targetSpeed.."km/h", 255,255,255,true)
...
addEventHandler("onClientPlayerVehicleExit", getLocalPlayer(), function(veh, seat)
	if (seat==0) then
		if (ccEnabled) then
			removeEventHandler("onClientRender", getRootElement(), cc)
			ccEnabled=false
			outputChatBox("#FFFFFFSabitleyici #FFFFFFEtkin Degil.", 255,255,255,true)
		end
	end
end)

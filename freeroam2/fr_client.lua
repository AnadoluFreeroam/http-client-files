...
wndOutfits = {
	...
	controls = {
		...
		{"br"},
		{'txt', id='outfitname', text='Kıyafet adı girin...',y=285, width=230},
		{"br"},
		{'btn', id='KAYDET', onclick=saveOutfit, width=90,y=315, height=22},
		{'btn', id='KALDIR', onclick=deleteOutfit,x=150, width=90,y=315, height=22},
		{"br"},
		{'btn', id='KAPAT', closeswindow=true,  width=230,y=340, height=22}
	},
...
wndSSS = {
    ...
    controls = {
		...
		{"br"},
		{"br"},
        {'lbl', id='sorucevap', text='» Üst listede en fazla sorulan sorular yer almaktadır.',x=15,y=290, width=505,height=110},
...
wndSkin = {
	...
	controls = {
		...
		{"br"},
		{'txt', id='skinid', text='', x=9,y=393,width=50},
		{'btn', id='SEÇ', onclick=applySkin,y=395,width=70, height=22},
		{'btn', id='KAPAT', closeswindow=true, y=395,width=70, height=22}
	},
	oncreate = skinInit
}
...
wndAnim = {
	...
	controls = {
		...
		{"br"},
		{'btn', id='BAŞLAT', onclick=applyAnimation, ClickSpamProtected=true, width=65, y=393,height=22},
		{'btn', id='DURDUR', onclick=stopAnimation, x=110,width=65, y=393,height=22},
		{'btn', id='KAPAT', closeswindow=true, x=215,width=65, y=393,height=22}
	}
}
    ...
    controls = {
        {
      	...
      	{'btn', id = 'KULLAN', onclick = applyWalkStyle,ClickSpamProtected=true,width=65, y=337,height=22 },
		{'btn', id='KALDIR', onclick=stopWalkStyle, x=92,width=65, y=337,height=22},
		{'btn', id='KAPAT', closeswindow=true, x=175,width=65, y=337,height=22}
    }
}
    ...
    controls = {
        {
          ...
          { 'btn', id = 'KULLAN', onclick = applyFightStyle,y=178, height=22, width=70 },
          { 'btn', id = 'KALDIR', onclick = stopFightStyle,x=90,y=178, height=22, width=70},
          { 'btn', id = 'KAPAT', closeswindow = true,x=170, y=178, height=22, width=70 }
    }
}
...
wndClothes = {
	...
	controls = {
		...
		{'br'},
		{'btn', text='Seçilen Kıyafeti Çıkar', id='addremove', height=22,y=428, width=330, onclick=applyClothes, ClickSpamProtected=true, x=10},
	    {'btn', id='Kıyafetleri Sıfırla', onclick=sifirlaClothes, height=22,y=453, width=330},
		{'btn', id='Menüyü Kapat', closeswindow=true, height=22, width=330,y=477}
	},
...
local fontss = guiCreateFont("lib/font.ttf",8)
...
function updatePlayerBlips()
	...
	for elem,player in pairs(g_PlayerData) do
		if not player.gui.mapBlip then
			...
			guiSetFont(player.gui.mapLabelShadow,fontss)
			...
			guiSetFont(player.gui.mapLabel, fontss)	
			...
			if elem == localPlayer then 
				elementerenkver(player.gui.mapBlip,"a83232")
			else 
				local r,g,b = getPlayerNametagColor(elem)
				local hex = rgb2hex(r,g,b)
				elementerenkver(player.gui.mapBlip,hex)
			end		
...
wndSetPos = {
	...
	width = g_MapSide + 20,
	controls = {
		{'img', id='map', src='img/map.png', y=35,width=g_MapSide, height=g_MapSide, onclick=fillInPosition, ondoubleclick=setPosClick, DoubleClickSpamProtected=true},
		{"br"},
		{"br"},
...
wndCreateVehicle = {
	...
	controls = {
		...
		{'btn', id="OLUŞTUR", onclick=createSelectedVehicle, ClickSpamProtected=true, width=290,y=428, height=22},
		{'btn', id='KAPAT', closeswindow=true, width=290,y=455, height=22}
	},
    oncreate = createInit,
}
...
setTimer(function()
	if hangidunyadadata(localPlayer) == "Freeroam" then
		guiSetEnabled(getControl(wndMain, 'kill'),true)
	else 
		guiSetEnabled(getControl(wndMain, 'kill'),false)
	end
end,100,0)
...
function killLocalPlayer()
	...
	errMsg("Tct dünyalarında intihar edemezsin!")
	end
end
...
wndBookmarks = {
	...
	controls = {
		...
		{'txt', id='bookmarkname', text='', width=225,y=325},
		{'btn', id='KAYDET', onclick=saveLocation, width=144, height = 25,y=325},
		{'btn', id='Seçilen Bölgeyi Sil', onclick=deleteLocation, width=225, height = 25,y=355},
		{'btn', id='KAPAT', closeswindow=true, width=144, height = 25,y=355}
	},
...
function farolay()
	if not isPedInVehicle(localPlayer) then errMsg("Farları açıp kapatman için bir araca sahip olmalısın.") return end
	if isInNightTime() then 
		guiCheckModeSetState(getControl(wndMain, 'fartik'),true)
		return 
	end
	if guiCheckBoxGetSelected(getControl(wndMain, 'fartik')) == false then
		...
		guiSetText(getControl(wndMain, 'fartik'),"Farları Aç")
		server.setVehicleOverrideLights(getPedOccupiedVehicle(localPlayer), 0)
	end
end
...
function aractamir()
	...
	if kapilar[getPedOccupiedVehicleSeat(localPlayer)] then 
		exports["vg-ustmesaj"]:sendClientMessage("#e3652b[ Oyuncu Paneli ]: #6b6b6bBu işlemi sadece araç sürücüsü yapabilir.")
		return 
	end
...
function olumsuzlukmod()
	if guiCheckBoxGetSelected(getControl(wndMain,"olumsuzluk")) == false then
		...
		exports["vg-progesbar"]:drawProgressBar("olumsuzluk", "Ölümsüzlük Modu Açılıyor", 240, 112, 53,5*1000)
	else
...
function arachasarsizlikmod()
	...
	if guiCheckBoxGetSelected(getControl(wndMain,"arachasarsizlik")) == false then
		...
		exports["vg-progesbar"]:drawProgressBar("ahasarsız", "Araç Dokunulmazlık Modu Açılıyor", 240, 112, 53,5*1000)
	else
...
function galeri()
	...
	exports["vg-kayitliaracgalerisi"]:galerigonder()
end
...
wndMain = {
	...
	width = 260,
	controls = {
		{'chk', id='olumsuzluk',text='Ölümsüzlük', onclick=olumsuzlukmod,x=10,y=42},
		{"chk"; id='arachasarsizlik',text = "Araç Hasarsızlığı", onclick = arachasarsizlikmod,x=125,y=42},
		{'btn', id='question', text='Sıkça Sorulan Sorular', window=wndSSS, width=240,r=157,g=69,b=204},
		...
		{'btn', id='', text='Ayarlar', onclick=ayarlarpanel,width=240},
		{'chk', id='falloff', text='Motordan Düşme', onclick=toggleFallOffBike},
		{'chk', id='disableknife', text='Bıçaklanma', onclick=toggleKnifing, x=145, y= 200},

		{'br'},
		{'btn', id='setpos', text='Harita', window=wndSetPos, width=240},
		{'btn', id='', text='Kıyafet Kaydet', window=wndOutfits},
		{'btn', id='', text='Yer Kaydet', window=wndBookmarks},
		{'br'},
		{'br'},
		{'btn', id='createvehicle', text='Araba Oluştur', window=wndCreateVehicle, width=240},
		...
		--{'btn', id='', text='Korna Sesleri',onclick=kornapanel, width=240},
		{'btn', id='', text='Motor Sesleri', onclick=motorsesleripanel, width=240},
		--{'btn', id='galeri', text="Kayıtlı Araçların / Galerin", onclick=galeri, width=240,r=157,g=69,b=204},
		--{'btn', id='', text='', onclick=modyoneticisipanel, width=240},

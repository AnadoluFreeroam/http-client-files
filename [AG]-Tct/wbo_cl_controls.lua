...
local guiControls = {
	...
	[ "scrollbar" ] = {
		create = function ( tbl, parent )
			...
			local control = {
				...
				element = guiCreateScrollBar ( 0.02, ypos + 0.045, 1, 0.035, true, true, parent ),
				min = tbl.value.min,
				max = tbl.value.max
			}
			...
			addEventHandler ( "onClientGUIScroll", control.element, 
				function ( )
					local value = guiToReal ( control.min, control.max, guiScrollBarGetScrollPosition ( control.element ) )
		...
		get = function ( self )
			return guiToReal ( self.min, self.max, guiScrollBarGetScrollPosition ( self.element ) )
		end,
	...
	[ "combobox" ] = {
		...
		get = function ( self )
			...
			return guiComboBoxGetItemText ( self.element, selectedItem )
		end
	},
	[ "gridlist" ] = {
		create = function ( tbl, parent )

...
function getPedWorldTarget ( ped )
	if isElement ( ped ) and getElementType ( ped ) == "player" then
		...
		if tx then
			...
			local _, _, _, _, _, _, _, _, _, _, _, worldModelID, worldModelPosX, worldModelPosY, worldModelPosZ = processLineOfSight ( sx, sy, sz, tx, ty, tz, true, false, false, false, false, true, false, true, localPlayer, true )

...
local pane = guiCreateWindow(panelX, panelY, panelW, panelH, 'Anadolu Freeroam - Kill Sırlaması', false)
if not pane then
  return false
end
...
local loadingText = guiCreateLabel(0, 0, panelW, panelH, 'Kill verileri yükleniyor.', false, pane)
...
addEventHandler('onClientPreRender', root, function()
    if getTickCount() - pointerTick > 1000 then
      ...
      if pointerCount == 3 then
        guiSetText(loadingText, 'Kill verileri yükleniyor.')
        pointerCount = 0
      end
...
addEventHandler("onClientKey",root,function(btn,press)
	if btn == "F7" and press then
		local suan = getTickCount()
		if not panelacik then
			if sonclick > suan then 
				outputChatBox("#ffffffKill tablosunu açmak için #0066ff10 #ffffffsaniye beklemelisiniz.",255,0,0, true)
				cancelEvent() 
				return 
			end
		end

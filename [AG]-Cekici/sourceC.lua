	...
	function ()
											...
											if tx then
												if i == 4 then
													if holder == localPlayer then
														outputChatBox("Kablo buradan geçemiyor!", 255, 255, 255, true)
													end
		...
		if isElement(myTowTruck) then
			...
			if getDistanceBetweenPoints3D(px, py, pz, tx, ty, tz) > 20 then
				outputChatBox("Kablo bu kadar uzağa ulaşamaz.", 255, 0, 0, true)
				...
				if state ~= "up" then
					...
					for k, v in pairs(keys) do
						if v == key then
							...
							if press then
								outputChatBox("Önce rampayı Çek.", 255, 255, 255, true)
							end
						end
					end
				end
						...
						if not rampInAnim[veh] then
							if key == "cardown" then
								if getElementData(veh, "packerState") == "down" then
									...
									outputChatBox("Önce rampayı çıkar.", 255, 255, 255, true)
								end
									...
									if getVehicleType(clickedElement) == "Automobile" then
										...
										if getDistanceBetweenPoints3D(px, py, pz, tx, ty, tz) <= 5 then
											...
											if isInsideColShape(detectorColShape, x, y, z) and dist <= 2.5 then
												...
												outputChatBox("Araba arabanın arkasında değil!", 255, 255, 255, true)
											end
										else
											outputChatBox("Araçtan çok uzaktasın!", 255, 255, 255, true)
										end
									else
										outputChatBox("Sadece araba bağlayabilirsin.", 255, 255, 255, true)
									end
								end
							end
						end
